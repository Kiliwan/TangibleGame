...
class ParticleSystem {
    ...
    void addParticle() {
        ...
        for(int i=0; i<NUM_ATTEMPTS; i++) {
            ...
            if(checkPosition(center)) {
                ...
                FullGame.totalScore-=cylinderPoint;
                FullGame.lastScore=-cylinderPoint;
                FullGame.score_history.add(FullGame.totalScore);
                break;
            }
        }
    }
    ...
    boolean run(PGraphics FullGameSurface,float velocity, color cylinderColor) {
      boolean toRet = false;
        //Check if the ball hit any cylinder and if so remove it
        for (int i = 0; i < particles.size(); i++) {
            fill(cylinderColor);
            FullGameSurface.shape(particles.get(i).sides());
            FullGameSurface.shape(particles.get(i).top());
            if(ball.checkCylinderCollision(particles.get(i), cylinderRadius)) {
                float velocityFactor = map(velocity,0,50,0,3);
                if(i == 0) {
                    particles.clear();
                    FullGame.totalScore+=bossPoint*velocityFactor;
                    FullGame.lastScore=bossPoint*velocityFactor;
                    FullGame.score_history.add(FullGame.totalScore);
                    toRet |= true;
                } else {
                    particles.remove(i);
                    FullGame.totalScore+=cylinderPoint*velocityFactor + 5;
                    FullGame.lastScore=cylinderPoint*velocityFactor + 5;
                    FullGame.score_history.add(FullGame.totalScore);
                }
            }
        }
