...
class ParticleSystem {
    ...
    void addParticle() {
        ...
        for(int i=0; i<NUM_ATTEMPTS; i++) {
            ...
            if(checkPosition(center)) {
                ...
                Game.totalScore-=cylinderPoint;
                Game.lastScore=-cylinderPoint;
                Game.score_history.add(Game.totalScore);
                break;
            }
        }
    }
    ...
    boolean run(PGraphics gameSurface,float velocity, color cylinderColor) {
      boolean toRet = false;
        //Check if the ball hit any cylinder and if so remove it
        for (int i = 0; i < particles.size(); i++) {
            fill(cylinderColor);
            gameSurface.shape(particles.get(i).sides());
            gameSurface.shape(particles.get(i).top());
            if(ball.checkCylinderCollision(particles.get(i), cylinderRadius)) {
                float velocityFactor = map(velocity,0,50,0,3);
                if(i == 0) {
                    particles.clear();
                    Game.totalScore+=bossPoint*velocityFactor;
                    Game.lastScore=bossPoint*velocityFactor;
                    Game.score_history.add(Game.totalScore);
                    toRet |= true;
                } else {
                    particles.remove(i);
                    Game.totalScore+=cylinderPoint*velocityFactor + 5;
                    Game.lastScore=cylinderPoint*velocityFactor + 5;
                    Game.score_history.add(Game.totalScore);
                }
            }
        }
