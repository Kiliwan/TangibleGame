...
void setup() {
  ...
  //pose = new TwoDThreeD(width,height,frameRate); // for camera
...
*/
  ...
  img = loadImage("board4.jpg"); // 2 at limit, 4 pretty okay
  noLoop();
}
...
/*
  ...
  img = thresholdHSB(img,100,140,80,255,30,160);
  //img = thresholdHSB(img,map(minH.getPos(),0,1,0,255),map(maxH.getPos(),0,1,0,255),map(minS.getPos(),0,1,0,255),map(maxS.getPos(),0,1,0,255),map(minB.getPos(),0,1,0,255),map(maxB.getPos(),0,1,0,255));
  img = convolute(img);
  img = blobDetection.findConnectedComponents(img, true);
  img = scharr(img);
  img = threshold(img,200);
  //img = threshold(img,(int)map(threshold.getPos(),0,1,0,255));
  ...
  List<PVector> quadLines = quadFinder.findBestQuad(lines, img.width, img.height, 1000000, 1000, true); // true for error messages
...
*/
  ...
  //pipeline = thresholdHSB(img,map(minH.getPos(),0,1,0,255),map(maxH.getPos(),0,1,0,255),map(minS.getPos(),0,1,0,255),map(maxS.getPos(),0,1,0,255),map(minB.getPos(),0,1,0,255),map(maxB.getPos(),0,1,0,255));
  pipeline = convolute(pipeline);
  pipeline = blobDetection.findConnectedComponents(pipeline, true);
  pipeline = scharr(pipeline);
  pipeline = threshold(pipeline,200);
  //pipelined = threshold(pipelined,(int)map(threshold.getPos(),0,1,0,255));
  ...
  for(int i=0;i<quadLines.size();++i){
    ...
    ellipse(quadLines.get(i).x,quadLines.get(i).y,30,30);
    popStyle();
    homogeneousCorners.add(new PVector(quadLines.get(i).x,quadLines.get(i).y,1f));
  }
  orientation = pose.get3DRotations(homogeneousCorners);
  
  println(degrees(orientation.x)+180);
  println(degrees(orientation.y));
  println(degrees(orientation.z));
  
  /*
  minH.display(scrollbarColor, scrollbarCursorColor_active, scrollbarCursorColor_inactive);
  minH.update();
  println(map(minH.getPos(),0,1,0,255));
  
  maxH.display(scrollbarColor, scrollbarCursorColor_active, scrollbarCursorColor_inactive);
  maxH.update();
  println(map(maxH.getPos(),0,1,0,255));
